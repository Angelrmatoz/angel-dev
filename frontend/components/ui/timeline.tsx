...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full font-sans relative" ref={containerRef}>
      ...
      <div
        ref={ref}
        className="relative container mx-auto pb-10 md:pb-20 lg:pb-32"
      >
        {data.map((item, index) => (
          <div
            key={index}
            className="relative flex justify-start pt-20 md:pt-40 md:gap-10 px-6 lg:px-20"
          >
            <div className="sticky flex flex-col z-40 items-start top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              {item.date && (
                <div
                  key={`date-desktop-${index}`}
                  className="hidden md:block md:pl-20 mt-2"
                >
        ...
        <div
          ...
          className="absolute left-14 lg:left-28 top-0 overflow-hidden w-0.5 bg-transparent mask-[linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]"
        >
          <motion.div
            style={{
